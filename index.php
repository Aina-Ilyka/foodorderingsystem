...
<?php include_once('includes/header.php');?>
        <section>
            <div class="block blackish opac50">
                <div class="fixed-bg" style="background-image: url(assets/images/design5.svg);"></div>
                <div class="restaurant-searching style2 text-center">
                    <div class="restaurant-searching-inner">
						<span>Delicious <i>Noodle</i> </span>
                        <h2 itemprop="headline">Order Dine-In & Pick-Up</h2>
                        <form class="restaurant-search-form2 brd-rd30" method="post" action="search-result.php">
           <input class="brd-rd30" type="text" placeholder="Noodle / Beverage Name" required="true"  name="searchdata">
           <button style="font-size: 1.4em;" class="fa fa-search brd-rd30 red-bg" type="submit" name="search"></button>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
    ...
    <?php if ($row['ItemQty'] == 'Available') { ?>
    <input type="hidden" name="foodid" value="<?php echo $row['ID'];?>"> 
	<input class="qty" name="foodqty" type="text" value="1">
    <h5 itemprop="headline"><a href="#" title="" style="color: red;"><?php echo $row['ItemQty'];?></a></h5>
                                               </p>
                                                <span class="price">RM <?php echo $row['ItemPrice'];?></span>

                              <?php if($_SESSION['cust_id']==""){?>
  <a class="log-popup-btn btn  pull-right red-bg brd-rd3" href="#" title="Login" style="color: white;">Order Now</a>
<?php } else {?>
<button type="submit" name="addcart" class="btn  pull-right red-bg brd-rd3" style="color: white;">Order Now</button>
                                                <?php } ?>
                                                <?php } else { ?>
                            <h5 itemprop="headline" style="color: red;"><?php echo ucfirst($row['ItemQty']); ?></h5>
                        <?php } ?>
                                                </p>
                                        </form>
                                            </div>
