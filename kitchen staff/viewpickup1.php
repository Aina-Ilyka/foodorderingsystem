...
<?php include_once('includes/leftbar.php');?>
       ...
       <div class="row border-bottom">
                ...
                <?php
                    // Retrieve values from the URL parameters sent via GET method
                    $date = isset($_GET['date']) ? $_GET['date'] : '';
                
                    $pagenr = isset($_GET['page-nr']) ? $_GET['page-nr'] : '';

                    $start=0;
                    $rows_per_page =1;

                    $records = $con->query("SELECT * FROM tblorderaddresses WHERE paystatus='Successful' AND ordertype='Pick-Up' AND OrderFinalStatus='Confirmed Order' AND DATE(OrderTime) = '$date' AND (
                        ADDTIME(CURTIME(), '00:10:00') >= pickuptime
                        OR pickuptime <= CURTIME()
                    ...
                    $result = $con->query("SELECT * FROM tblorderaddresses WHERE paystatus='Successful' AND ordertype='Pick-Up' AND OrderFinalStatus='Confirmed Order' AND DATE(OrderTime) = '$date' AND (
                        ADDTIME(CURTIME(), '00:10:00') >= pickuptime
                        OR pickuptime <= CURTIME()
                    )ORDER BY OrderTime ASC LIMIT $start,$rows_per_page ");
                                ...
                                <div class="ibox">
                                    <div class="ibox-content">
                                                ...
                                                <div class="col-6" style="margin-top:1.3%">
                                                    ...
                                                    <table border="1" class="table table-bordered mg-b-0">
                                                        ...
                                                        while ($row1 = mysqli_fetch_array($query)) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <?php 
                                                                // Check if the category is not 'Drink' to display the checkbox
                                                                if ($row1['CategoryName'] !== 'Drink') { ?>
                                                                    <td style="text-align: center;">
                                                                        <input id="food_done_<?php echo $row1['FoodId'] . '_' . $oid; ?>" type="checkbox" name="food_done[]" value="<?php echo $row1['FoodId']; ?>" onchange="updateStatus(this, '<?php echo $row1['FoodId']; ?>', '<?php echo $oid; ?>')">
                                                                    </td>
                                                                <?php } else {
                                                                    $drinkCategoryExists = true; // Set flag to true if 'Drink' category exists
                                                                } ?>
                                                    ...
                                                    <div class="row pagination-section">
                                                        ...
                                                        <div class="col-lg-12 text-right" style="margin-top:3%;"> <!-- Aligning content to the right -->
                                                            ...
                                                            // Retrieve today and pickup values from the URL parameters
                                                            $today = isset($_GET['date']) ? $_GET['date'] : '';
                                                            $savedStatus = isset($_GET['pickup']) ? $_GET['pickup'] : '';
                                                            if (!isset($pagenr) || ($pagenr < $pages) || ($pages == 1)) {
                                                                $nextPage = isset($pagenr) ? ($pagenr + 1) : 2;
                                                                $nextPage = ($nextPage > $pages) ? 1 : $nextPage;
                                                            ?>
                                                               <a href="viewpickup1.php?date=<?php echo $today; ?>&pickup=<?php echo $savedStatus; ?>&page-nr=1" class="next-button">First Order</a>
                                                               <a href="viewpickup1.php?date=<?php echo $today; ?>&pickup=<?php echo $savedStatus; ?>&page-nr=<?php echo $nextPage; ?>" class="next-button">Next</a>
                                                            <?php } else { ?>
                                                                <a href="viewpickup1.php?date=<?php echo $today; ?>&pickup=<?php echo $savedStatus; ?>&page-nr=1" class="next-button">First Order</a>
                                                            <?php } ?>
                                                        </div>
                                        ...
                                        <?php }} else {
                                                    ...
                                                    echo '<script>
                                                    if(confirm("All food orders for today have been completed. Click OK to go to dashboard.")) {
                                                        var pickupValue = "' . (isset($_GET['pickup']) ? $_GET['pickup'] : '') . '";
                                                        window.location.href = "dashboard.php?pickup=" + pickupValue;
                                                    }
                                                </script>';
                                                        ...
                                                        <div class="row justify-content-center">
                                                            <div class="col-md-6">
                                                                <div class="alert alert-info text-center">
                                                                    <p><strong>All food orders for today have been completed.</strong></p>
                                                    ...
                                                    echo '<a href="dashboard.php?pickup=' . $pickup . '" class="btn btn-primary">OK</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>';}
    ...
    if (this.readyState == 4 && this.status == 200) {
        ...
        if (response.success) {
            ...
            // Get the current URL
                ...
                // Get the value of the 'page-nr' parameter from the URL
                var currentPageNumber = currentURL.searchParams.get("page-nr");

                // Redirect to the same page with the current page number
                window.location.href = "viewpickup1.php?date=<?php echo $today; ?>&pickup=<?php echo $savedStatus; ?>&page-nr=" + currentPageNumber;
        } else {
