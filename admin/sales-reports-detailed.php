...
$y2=date("Y",$month2);
    ?>
    <h4 class="header-title m-t-0 m-b-30">Monthly Sales Report</h4>
...
$ret=mysqli_query($con,"select month(OrderTime) as lmonth,year(OrderTime) as lyear,
    ...
    and tblorderaddresses.paystatus='$fstatus'  group by lmonth,lyear");
$cnt=1;
...
<?php
$ret=mysqli_query($con,"select year(OrderTime) as lyear,sum(ItemPrice*tblorders.FoodQty) as totalitmprice from tblorders join tblorderaddresses on tblorderaddresses.Ordernumber=tblorders.OrderNumber join tblfood on tblfood.ID=tblorders.FoodId where year(tblorderaddresses.OrderTime) between '$fdate' and '$tdate' and tblorderaddresses.paystatus='$fstatus' group by lyear");
$cnt=1;
while ($row=mysqli_fetch_array($ret)) {
...
<script>
    ...
    function printTable() {
        ...
        // Add the title
        printWindow.document.write('<h4 align="center" style="color: blue;"><?php echo $rtype == "mtwise" ? "Monthly Sales Report" : "Yearly Sales Report"; ?> from <?php echo $rtype == "mtwise" ? $m1 . "-" . $y1 : $y1; ?> to <?php echo $rtype == "mtwise" ? $m2 . "-" . $y2 : $y2; ?></h4>');
